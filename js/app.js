...
function submitEnquiryForm(e) {
  ...
  const service = document.getElementById('enquiry-service-select').value;
  const modal = document.querySelector('.enquiry-modal');
  if (!modal) return;

  // Data submission handled by forms.js interceptor
  ...
  if (!targetCourse) {
    ...
    if (hash === '#ai-development') {
      ...
      const slugMap = {
        ...
        'game-development': 'Game Development (3D & 2D)'
      };
...
function submitSolutionEnquiry(e) {
  ...
  var company = document.getElementById('sol-enq-company')?.value.trim();
  var message = document.getElementById('sol-enq-message')?.value.trim();
  var solution = document.getElementById('sol-enq-solution').value;
  var package = document.getElementById('sol-enq-package').value;
...
// Offer form data submission handled by forms.js interceptor
...
function init() {
  ...
  document.getElementById('contact-form')?.addEventListener('submit', function (e) {
    ...
    // Actual submission (Google Sheets)
    ...
    setTimeout(() => {
      ...
      if (contactCard) {
        contactCard.innerHTML = `
          <div class="enquiry-success" style="padding:4rem 2rem;text-align:center;">
            <div class="enquiry-success-icon" style="margin-bottom:2rem;">
            ...
            <h2 style="font-family:'Playfair Display',serif;font-size:2rem;margin-bottom:1rem;">Thank You, ${name}!</h2>
            <p style="color:#64748b;font-size:1.1rem;line-height:1.7;margin-bottom:2.5rem;">Your message has been received successfully. Our team will review your enquiry and get back to you within 24 hours.</p>
            <button class="btn btn-primary" onclick="location.hash='#home'" style="padding:0.85rem 3rem;border-radius:0.75rem;">Done</button>
          </div>
        `;
      }
  ...
  document.addEventListener('submit', function (e) {
    ...
    if (typeof submitToSheets === 'function') {
      submitToSheets('enquiry', {
        ...
        Phone: tPhone,
        Company: tCompany,
        ServiceAI: 'Corporate Training: ' + (document.getElementById('t-course')?.value || 'General'),
        MessageAI: document.getElementById('t-message')?.value || '',
...
function validateCareerStep(step) {
  ...
  if (step === 1) {
    ...
    if (!resume.value.trim()) { resume.classList.add('error-border'); isValid = false; }
...
function submitCareerForm() {
  if (!validateCareerStep(4)) return false;
  ...
  // Data submission is handled entirely by forms.js interceptor
  // (Google Form + Google Sheets + Resume upload)
...
function renderSolutionDetail(data) {
  ...
  if (data.coursesPricing && data.coursesPricing.length > 0) {
    ...
    html += '<section class="solution-section" style="background:var(--bg-light);"><div style="max-width:72rem;margin:0 auto;text-align:center;">';
    html += '<h2 class="section-title reveal">' + data.name + '</h2>';
